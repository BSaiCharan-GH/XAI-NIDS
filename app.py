...
DB_PATH = Path(__file__).resolve().with_name("detections.db")
        ...
        with sqlite3.connect(DB_PATH) as conn:
            conn.execute(
                ...
                INSERT INTO detection_history (
                    ...
                    destination_port, protocol, predicted_class,
                    confidence, severity, detection_method,
                    relevant_features, shap_explanation
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    float(report.get("ts") or time.time()),
                    str(report.get("source_ip") or report.get("source") or "").strip() or None,
                    str(report.get("destination_ip") or report.get("destination") or "").strip() or None,
                    int(report.get("source_port") or 0) if str(report.get("source_port") or "").isdigit() else None,
                    int(report.get("destination_port") or 0) if str(report.get("destination_port") or "").isdigit() else None,
                    str(report.get("protocol") or "").strip() or None,
                    str(report.get("threat_type") or report.get("prediction") or "UNKNOWN").strip() or "UNKNOWN",
                    float(report.get("confidence") or 0.0),
                    str(report.get("severity") or "MEDIUM").strip() or "MEDIUM",
                    str(report.get("detection_method") or "CICIDS-2017 Random Forest + Scapy").strip() or "CICIDS-2017 Random Forest + Scapy",
                    json.dumps(report.get("features") or {}, sort_keys=True),
                    json.dumps(report.get("shap") or [], sort_keys=True),
                ),
            )
            conn.commit()
    except Exception:
...
def api_system_status():
    ...
    return jsonify({
        ...
        "database": {"ready": DB_PATH.exists(), "path": str(DB_PATH)},
...
def api_detection_history():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if requested_class == "ALL":
            rows = conn.execute("SELECT * FROM detection_history ORDER BY timestamp DESC LIMIT 200").fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM detection_history WHERE predicted_class = ? ORDER BY timestamp DESC LIMIT 200",
                (requested_class,),
            ).fetchall()
    return jsonify([
        {
            ...
            "relevant_features": json.loads(row["relevant_features"] or "{}"),
            "shap_explanation": json.loads(row["shap_explanation"] or "[]"),
        }
        for row in rows
    ])
...
def _record_activity(sid, event_name, payload_size=0, packet_kind="control", details=None):
    ...
    with lock:
        ...
        packet = {
            ...
            "status": "blocked" if throttled_until.get(sid, 0) > now else "accepted",
        }
...
def _build_report_pdf():
    ...
    with lock:
        ...
        series = [
            sum(1 for t in REQUESTS_LOG if age <= now - t < age + 1)
    ...
    cards = [
        ...
        ("DOS EVENTS", counts["flood"], (0.78, 0.25, 0.34)),
        ("PORT SCANS", counts["port_scan"], (0.85, 0.48, 0.18)),
    ...
    _pdf_text(c, 42, 577, "Requests per second, oldest bucket on the left and newest on the right.", 9, (0.35, 0.42, 0.50))
    ...
    mix = [("chat", counts["chat"]), ("file", counts["file"]), ("dos", counts["flood"]), ("port scan", counts["port_scan"]), ("control", counts["control"])]
    ...
    if not alert_snapshot:
        ...
        for alert in alert_snapshot[:8]:
            ...
            _pdf_text(c, 55, y - 38, f"Client: {alert.get('client', 'unknown')}{confidence}   Mitigation: 10s rate limit", 9)
    ...
    for packet in packet_snapshot[:32]:
        ...
        _pdf_text(c, 105, y, str(packet["source"])[:16], 8)
