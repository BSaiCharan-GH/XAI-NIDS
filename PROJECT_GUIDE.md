...
The project keeps communication state in memory. At startup the
...
the server process and dashboard session are running.
...
6. SHAP attribution, threat assessment, and dashboard publication happen only
   after captured-flow classification.
7. The monitor displays the selected report in the Attack Explanation Panel,
...
- Alert count
- Chat, file, flood, and control event mix
- Readable 30-second traffic graph with axes and grid lines
- Recent detections and confidence values
- Mitigation state
